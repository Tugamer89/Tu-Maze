...
void Shaders::load_fallback() {
    ...
    if (!compile_attach_link(&vertex_source, &fragment_source)) {
        throw exceptions::ShaderException("Critical error: Failed to compile fallback shaders.");
    }
}

void Shaders::load(const std::string& vertex_file, const std::string& fragment_file) {
...
void Shaders::reload(const std::string& vertex_file, const std::string& fragment_file) {
    clean();
    try {
        load(vertex_file, fragment_file);
    } catch (const exceptions::ShaderException& e) {
...
bool Shaders::compile_attach_link(const char** vertex_source_ptr,
