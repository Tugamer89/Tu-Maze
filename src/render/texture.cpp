...
TextureQuality Texture::autoDetectQuality() {
...
Texture::Texture(const std::string& filepath, bool sRGB /*= false*/) {
    if (maxSupportedAnisotropy < 0.0f) {
        glGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &maxSupportedAnisotropy);
    }
    ...
    if (!image.loadFromFile(filepath)) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, magenta.data());
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, image.getSize().x, image.getSize().y, 0, GL_RGBA,
...
void Texture::setQuality(TextureQuality quality) const {
    glBindTexture(GL_TEXTURE_2D, m_textureID);
...
void Texture::setGlobalQuality(TextureQuality quality) {
