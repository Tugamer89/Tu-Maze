...
void Minimap::setupProjection(const Scene& scene, const Gui& gui, const glm::vec3& camPos) const {
    ...
    glm::mat4 pr = glm::ortho(-gui.getMinimapZoom(), gui.getMinimapZoom(), -gui.getMinimapZoom(),
                              gui.getMinimapZoom(), 0.1f, 100.0f);
...
void Minimap::drawPlayerMarker(const Scene& scene, const Gui& gui, GLint mLoc, GLint cLoc,
                               const glm::vec3& camPos) const {
    if (!playerMesh) return;

    glm::mat4 model = glm::translate(glm::mat4(1.0f), camPos);
    model =
        glm::rotate(model, glm::radians(-scene.getCamera().getYaw() - 90.0f), glm::vec3(0, 1, 0));

    float markerScale = gui.getMinimapZoom() * 0.15f;
    model = glm::scale(model, glm::vec3(markerScale));

    glUniformMatrix4fv(mLoc, 1, GL_FALSE, glm::value_ptr(model));
    glUniform3f(cLoc, 1.0f, 0.2f, 0.2f);
...
void Minimap::draw(const Scene& scene, const Gui& gui) {
